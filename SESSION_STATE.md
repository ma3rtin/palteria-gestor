...
| **26 — Limpieza de peso y ordenación de pedidos por hora** | ✅ | Remoción de la información de peso en etiquetas y portapapeles. Registro de la hora ART de creación en pedidos/cobros, visualización en el desglose de entregas y cobros, y ordenación por defecto de forma descendente (los últimos creados arriba) tanto en la lista diaria como en el dashboard de inicio (Home). |
...
| **33 — Lotes por fecha, Pipeline de CI y Bug de Cobranza** | ✅ | Creación de rama, refactorización de Server Actions de Productos a `src/actions/productos.ts` con 9 tests unitarios, input de fecha de lote opcional en el creador, visualización compacta de lote en la tabla de productos y selector de carga de nuevo pedido. Reubicación del despliegue de migraciones a GitHub Actions condicional a main (evitando errores P1001 en Vercel) y desactivación temporal del linter en la pipeline. Corrección de bug lógico en creación de cobros que hacía que se registraran en base de datos con `$ 0` pagados y en estado `PENDIENTE`. |
...
| **35 — Clientes: Inactividad, Tendencias y Volumen** | ✅ | Pestañas Activos/Inactivos por último pedido o cobro (2 semanas). Tendencias rodantes con iconos de incremento/reducción de cajas en las últimas 2 semanas. Clasificación de volumen discreta ([0-10], [10-20], etc.) y promedio semanal en detalle del cliente. Filtro de volumen en buscador. API actualizada, tests en Vitest pasados y build exitoso. Corrección de bug y unificación del marcado de pedidos pagados (tanto desde entregas como desde la sección de cobranzas individuales o grupales "Cobrar todo") para que asiente consistentemente los saldos cobrados en pagosParciales con la fecha actual, agregando revalidaciones de caché de ficha de cliente en todas las mutaciones. |
