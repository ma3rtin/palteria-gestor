...
export default async function EditarClientePage({ params }: Props) {
  ...
  return (
    <div className="p-8 max-w-2xl mx-auto">
      <div className="mb-6">
        <a href={`/clientes/${id}`} className="text-xs text-[#9aab9d] hover:text-[#ea580c]">
          ← {cliente.nombre}
        </a>
        <h1 className="text-2xl font-bold text-[#1a2419] mt-1">Editar cliente</h1>
      </div>

      <form action={action} className="bg-white rounded-lg border border-[#dde6de] p-6 flex flex-col gap-5">
        <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-[#1a2419] mb-1">Dirección</label>
            <input
              name="direccion"
        ...
        <div className="flex gap-3 pt-2">
          ...
          <a href={`/clientes/${id}`} className="px-6 py-2 rounded-lg text-sm text-[#5a6b5c] border border-[#dde6de] hover:border-[#9aab9d] transition-colors">
            Cancelar
          </a>
        </div>
      </form>
