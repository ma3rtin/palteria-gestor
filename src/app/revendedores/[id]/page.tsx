...
export default async function RevendedorDetallePage({ params, searchParams }: Props) {
  ...
  const montoCalculado = pedidos.reduce((s, p) => s + p.comisionRevendedor, 0);
  ...
  const filas = pedidos.map((p) => (
    <tr key={p.id} className="border-b border-[#22252e] last:border-0 hover:bg-[#22252e]">
      <td className="px-4 py-3 text-[#9ca3af]">{formatearFechaCorta(p.fecha)}</td>
      ...
      <td className="px-4 py-3 text-right font-semibold text-[#4ade80]">{formatearPeso(p.comisionRevendedor)}</td>
    </tr>
  ));
